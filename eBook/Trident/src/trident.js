...
function beginWith(string, begin) {
	return string.slice(0, begin.length) == begin;
}
...
class TridentPen {
	...
	write(text){
		this.te.auto_text(text);
	}

	center_text(text) {
		this.te.center_text(text);
	}
...
class TridentLayout {
	...
	center_text(text) {
		...
		var left = (max_width - text_width) / 2;
		tp._moveTo([left + tp.config.margin.left, tp.config.pen.pos[2]]);
		tp.drawText([text]);
	...
	text(text, meansure) {
		...
		var meansure_start = tp.config.pen.pos.slice();
		...
		if(meansure){
			var meansure_end = tp.config.pen.pos.slice();
			tp.config.pen.pos = meansure_start;
			return [meansure_start, meansure_end];
		}
	}
	
	auto_text(text){
		if(this.text(text, true)[1][2]<tp.config.paper_size[1])
			this.text(text);
		else{
			tp.newPage();
			this.text(text);
...
class Trident {
	...
	exec(code) {
		var fname = (code + "[").match(/\\.+?(\[|\{)/g)[0];
		fname = fname.slice(1, fname.length - 1);
		var para = code.slice(fname.length + 1);
		this.exec_list[fname](para);
	}
}